...
function prettyGraphics(kit) {
    ...
    // Labels
    doc.fontSize(16).fillColor(OP_DARK_RED).text("1Password Account", 0, 318, { align: "center" }).fontSize(10).fillColor("#333").text("SIGN-IN ADDRESS", LEFT_MARGIN + 28, 342).text("EMAIL ADDRESS", LEFT_MARGIN + 28, 398).text("ACCOUNT KEY", LEFT_MARGIN + 28, 454).text("MASTER PASSWORD", LEFT_MARGIN + 28, 510);
    ...
    doc.fillColor("#333").font("Helvetica-Bold").fontSize(14).text("Account code", 403, 610).moveDown(1).font("proxima").text("Scan this code from the 1Password apps to set up your account quickly and easily.", { width: 174 });

    // User input
    doc.fontSize(13).fillColor(OP_DARK_RED).text("Created for " + kit.name + " on " + (0, _moment2.default)().format('MMMM Do, YYYY') + ".", 0, 128, { align: "center" });
    // Account URL
